...
def test_identical_full_reread_is_suppressed() -> None:
    def body() -> None:
        Path("m.txt").write_text("a\nb\nc\n", encoding="utf-8")
        tools = HashlineTools()
        tools.execute("beginTransaction", {"file": "m.txt"})
        again = tools.execute("beginTransaction", {"file": "m.txt"})
        assert_true("Skipped re-opening" in again, f"redundant open suppressed: {again}")
        assert_true("unchanged since your last beginTransaction" in again, f"explains why: {again}")
        assert_true(not _has_body(again), f"no file body in a suppressed open: {again}")
...
def test_force_arg_bypasses_suppression() -> None:
    def body() -> None:
        ...
        tools = HashlineTools()
        tools.execute("beginTransaction", {"file": "m.txt"})
        forced = tools.execute("beginTransaction", {"file": "m.txt", "_force": True})
...
def test_read_again_after_suppression_serves() -> None:
    # Prompt-free escape: the second redundant open of an unchanged file is served.
    def body() -> None:
        Path("m.txt").write_text("a\nb\nc\n", encoding="utf-8")
        tools = HashlineTools()
        tools.execute("beginTransaction", {"file": "m.txt"})
        first = tools.execute("beginTransaction", {"file": "m.txt"})
        assert_true("Skipped re-opening" in first, "first redundant open suppressed")
        second = tools.execute("beginTransaction", {"file": "m.txt"})
        assert_true("Skipped re-opening" not in second, f"asking again serves it: {second}")
        assert_true(_has_body(second), "served open has the body")
...
def test_small_change_under_gate_is_suppressed_with_drift_note() -> None:
    def body() -> None:
        ...
        # note warns ids near the edit may be stale.
        Path("m.txt").write_text("line1\nlineX\nline3\nline4\nline5\n", encoding="utf-8")
        again = tools.execute("beginTransaction", {"file": "m.txt"})
        assert_true("Skipped re-opening" in again, f"small change suppressed: {again}")
...
def test_edit_invalidates_suppression() -> None:
    ...
    # beginTransaction returns the current file (the common open → edit → re-open flow).
    def body() -> None:
        Path("m.txt").write_text("aaaa\nbbbb\ncccc\n", encoding="utf-8")
        tools = HashlineTools()
        shown = tools.execute("beginTransaction", {"file": "m.txt"})
        tools.execute("edit", {"file": "m.txt", "id": _line_id(shown, 2), "content": "dddd"})
        reread = tools.execute("beginTransaction", {"file": "m.txt"})
...
def test_deleted_file_after_read_is_not_suppressed() -> None:
    def body() -> None:
        ...
        tools.execute("beginTransaction", {"file": "m.txt"})
        os.remove("m.txt")
        result = tools.execute("beginTransaction", {"file": "m.txt"})
