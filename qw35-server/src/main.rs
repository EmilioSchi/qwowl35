...
mod tests {
    ...
    fn defaults_from(args: &[&str]) -> GenerationDefaults {
        let owned: Vec<String> = args.iter().map(|s| s.to_string()).collect();
        match Config::parse(owned).expect("parse ok") {
            Command::Run(c) => c.generation_defaults,
    ...
    #[test]
    fn mode_seeds_full_profile() {
        let d = defaults_from(&["--mode", "thinking-coding"]);
        assert_eq!(d.temperature, 0.6);
        assert_eq!(d.top_p, 0.95);
        assert_eq!(d.presence_penalty, 0.0);
        // thinking presets keep the windowed repetition loop-breaker on.
        assert_eq!(d.repetition_penalty, 1.1);
        assert!(d.enable_thinking);
    }

    #[test]
    fn explicit_flag_overrides_mode_regardless_of_order() {
        let before = defaults_from(&["--temperature", "0.1", "--mode", "thinking-coding"]);
        let after = defaults_from(&["--mode", "thinking-coding", "--temperature", "0.1"]);
        // explicit --temperature wins in both orders
        assert_eq!(before.temperature, 0.1);
        assert_eq!(after.temperature, 0.1);
        // non-overridden preset fields still applied
        assert_eq!(before.presence_penalty, 0.0);
        assert_eq!(after.presence_penalty, 0.0);
        assert!(before.enable_thinking);
        assert!(after.enable_thinking);
    }

    #[test]
    fn invalid_mode_is_rejected() {
        let owned = vec!["--mode".to_string(), "bogus".to_string()];
        assert!(Config::parse(owned).is_err());
    }
}
